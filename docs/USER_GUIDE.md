...
8. 项目选择器默认显示公开安全的 `example` 项目；如需运行自己的项目，再手动选择
   `projects/<name>/`
9. UI 会自动读取本机 Ollama 已安装模型，并显示在 `Installed Ollama models`
10. 如果刚安装或删除模型，点击 `Refresh models` / `刷新模型`
11. 在下拉框选择已安装模型
12. 如果列表里没有你的模型，在 `Manual model name` / `手动输入模型名` 输入完整模型名
13. 确认 `Effective model` / `实际使用模型`
14. 点击 `Run Diagnostic / Run Normal / Run Continuous / Resume`
15. UI 会自动传参：`--model <effective_model>`
